...
class CovidData:
    ...
    def _compute_rel_dataframe(self, df):
        ...
        df_tmp = df.drop(['Population','Residents per case'], axis=1)       # delete columns from dataframe
    ...
    def _compute_agr_dataframe(self, df):
        ...
        df_agr.insert(2, 'AGR (mean value)', list(si_pct))                  # save mean values
        df_agr = df_agr.iloc[:, list(range(3))]                             # drop evrything after column 'Mean Values'
    ...
    def _check_for_update(self):
        ...
        if time() - self.last_update_time > 4 * 3600:
            ...
            # Insert population column to dataframes
    ...
    def _prune_dataframe(self, df, cases, days):
        ...
        '''
        df = df.iloc[:, [0, 1] + list(range(-days, 0))]
        if cases > 0:
    ...
    def compute_abs_values(self, days, cases, country=""):
        ...
        dat_start = df_abs_tmp.columns[2]
